...
const Pricing = () => {
  ...
  return (
    <div className='md:px-14 p-4 max-w-s mx-auto mx-4 py-10 my-16 bg-teal-50 rounded-br-[80px] rounded-tl-[80px]' id='pricing'>
      <div className='text-center'>
        <h2 className='md:text-5xl text-3xl font-extrabold text-primary mb-2'>{t('pricing.title')}</h2>
        <p className='text-tertiary md:w-1/3 mx-auto px-4'>{t('pricing.description')}</p>

        {/* Toggle Button */}
        <div className='mt-12'>
          <label htmlFor='toggle' className='inline-flex items-center cursor-pointer'>
            <span className='mr-8 text-2xl font-semibold'>{t('pricing.monthly')}</span>
            <div className='w-14 h-6 bg-gray-300 rounded-full transition duration-200 ease-in-out'>
              <div className={`w-6 h-6 rounded-full transition duration-200 ease-in-out ${isYearly ? "bg-primary ml-8" : "bg-gray-500"}`}></div>
            </div>
            <span className='ml-8 text-2xl font-semibold'>{t('pricing.yearly')}</span>
          </label>
          <input type='checkbox' id='toggle' className='hidden' checked={isYearly} onChange={() => setIsYearly(!isYearly)} />
        </div>
      </div>
      ...
      <div className='grid sm:grid-cols-2 lg:grid-cols-3 gap-10 mt-8 md:w-11/12 m-auto'>
        {packages.map((pkg, index) => (
          <div key={index} className='border border-gray-400 py-10 md:px-6 px-4'>
            <h3 className='text-3xl font-bold text-center text-primary'>{pkg.name}</h3>
            <p className='text-tertiary text-center my-5'>{pkg.description}</p>
            <p className='mt-5 text-center text-secondary text-4xl font-bold'>
              {isYearly ? `${pkg.yearlyPrice}` : `${pkg.monthlyPrice}`}
              <span className='text-base text-tertiary font-medium'>/{isYearly ? t('pricing.yearly') : t('pricing.monthly')}</span>
            </p>
            <ul className='mt-4 space-y-2 px-4'>
              <li className='flex gap-3 items-center'>
              <IoIosCheckmarkCircle />{pkg.tick1}
              </li>
              <li className='flex gap-3 items-center'>
              <IoIosCheckmarkCircle />{pkg.tick2} 
              </li>
              <li className='flex gap-3 items-center'>
              <IoIosCheckmarkCircle />{pkg.tick3} 
              </li>
              <li className='flex gap-3 items-center'>
              <IoIosCheckmarkCircle />{pkg.tick4}
              </li>
            </ul>
            <div className='w-full mx-auto mt-8 flex items-center justify-center'>
              <button className='btnPrimary'>{pkg.buttonText}</button>
            </div>
          </div>
        ))}
      </div>
