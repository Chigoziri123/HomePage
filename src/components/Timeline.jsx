...
const Timeline = () => {
  const { t, i18n } = useTranslation(); // Initialize the translation hook and i18n for language selection
  ...
  return (
    <div className='md:px-14 p-4 max-w-s mx-auto'>
      <div className='flex flex-col md:flex-row-reverse justify-between items-center gap-8 bg-gray-200 min-h-screen flex justify-center p-20'>
        ...
        {/* Content */}
        <div className='md:w-2/5'>
          <h2 className='text-3xl text-primary font-bold lg:w-full mb-7'>
            {t('timeline.title')} {/* Access translated title */}
          </h2>
          <ul>
            {t('timeline.steps', { returnObjects: true }).map((step, index) => (
              <li key={index} className='relative flex gap-6 pb-5 items-baseline'>
                <div className='before:absolute before:left-[5.5px] before:h-full before:w-[1px] before:bg-gray-900'>
                  <BsFillGiftFill className='pr-1' />
                </div>
                <div>
                  <p className='text-primary text-2xl font-2xl'>{step.title}</p>
                  <p className='text-tertiary mt-2'>{step.content}</p>
                </div>
              </li>
            ))}
          </ul>
