    ...
    <div className='gradientBg rounded-xl rounded-br-[80px] md:p-9 px-4 py-9'>
      <div className='flex flex-col md:flex-row-reverse justify-between items-center gap-10'>
        ...
        <motion.div
          ...
          className='md:w-3/5'>
          <h2 className='md:text-7xl text-4xl font-bold text-primary mb-6 leading-relaxed'>{t('banner.title')}</h2>
          <p className='text-[#EBEBEB] text-2xl mb-8'>{t('banner.description')}</p>

          {/* Fix button size and responsiveness */}
          <div
            className='flex flex-col items-start xs:flex-row xs:items-center xs:w-full gap-4 w-48   md:space-x-5 text-sm'
          >
            <button className='btnPrimary'>{t('banner.callDoctor')}</button>
            <button className='btnPrimary lg:m-8'>{t('banner.chatDoctor')}</button>
          </div>
        </motion.div>
      </div>
    </div>
